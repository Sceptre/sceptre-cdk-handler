...
class TestCommandChecker(TestCase):
    def setUp(self):
        ...
        self.subprocess_run_func = create_autospec(
            subprocess.run, side_effect=self.subprocess_run
        )
        self.logger = Mock(Logger)

        self.commands = {"exists": "/path/to/executable"}

        self.local_npm_packages = {"local": True}

        self.global_npm_packages = {"global": True}
