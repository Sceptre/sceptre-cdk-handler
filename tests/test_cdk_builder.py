...
class TestBootstrappedCdkBuilder(TestCase):
    def setUp(self):
        ...
        self.manifest = Mock(
            **{
                ...
                "contents.docker_images": {"hashhashhash": {"blah": "blah"}},
                "contents.files": {
        ...
        self.assembly.artifacts = [Mock(name="irrelevant_artifact"), self.manifest]
    ...
    def build(self):
        return self.builder.build_template(self.context, self.sceptre_user_data)
    ...
    def test_build_template__synthesized_assembly_has_no_manifest_artifact__raises_sceptre_exception(
        self,
    ):
    ...
    def test_build_template__synthesized_assembly_has_only_template_asset__does_not_publish_assets(
        self,
    ):
    ...
    def test_build_template__runs_cdk_assets_publish_on_the_asset_artifacts_file_with_correct_envs(
        self,
    ):
        self.build()
    ...
    def test_build_template__running_cdk_assets_command_raises_called_process_error__reraises_cdk_invocation_error(
        self,
    ):
        self.subprocess_run.side_effect = subprocess.CalledProcessError(
            1, "bad command"
        )
...
class TestBootstraplessCdkBuilder(TestCase):
    def setUp(self):
        ...
        self.bootstrapless_config = {"file_asset_bucket_name": "my_bucket"}
    ...
    def test_build_template__no_synthesizer_config__instantiates_synthesizer_with_no_kwargs(
        self,
    ):
    ...
    def test_build_template__instantiates_synthesizer_with_synthesizer_config_kwargs(
        self,
    ):
        self.builder.build_template(self.context, self.sceptre_user_data)
        self.synthesizer_class.assert_any_call(**self.bootstrapless_config)

    def test_build_template__invalid_synthesizer_arguments__raises_template_handler_arguments_invalid_error(
        self,
    ):
...
class TestCdkJsonBuilder(PyFakeFsTestCase):
    def setUp(self):
        ...
        self.subprocess_run = create_autospec(
            subprocess.run, side_effect=self.fake_subprocess_run
        )
        self.bootstrapless_config = {"file_asset_bucket_name": "my_bucket"}
    ...
    def test_build_template__bootstrapless_config_specified__synthesizes_template_with_bootstrapless_envs(
        self,
    ):
        self.builder.build_template(self.context, self.sceptre_user_data)
        expected_envs = {
    ...
    def test_build_template_asset_manifest_only_has_template__does_not_publish_assets(
        self,
    ):
        self.builder.build_template(self.context, self.sceptre_user_data)
        self.assertFalse(self.artifacts_published)

    def test_build_template__asset_manifest_has_other_file_assets__publishes_artifacts_with_connection_manager_envs(
        self,
    ):
    ...
    def test_build_template__asset_manifest_has_image_assets__publishes_artifacts_with_connection_manager_envs(
        self,
    ):
    ...
    def test_build_template__returns_template_from_json(self):
        ...
        self.assertEqual(self.expected_template, result)
    ...
    def test_build_template__running_cdk_synth_command_raises_called_process_error__reraises_cdk_invocation_error(
        self,
    ):
